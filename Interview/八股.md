...
UniquePtr【move只是把对象转成右值，并**触发**移动构造/赋值函数；和**编译器**没关系，执行**资源转移**的是接收右值参数的移动xx函数内部的逻辑】
...
### delete this 什么时候会有问题？

在 C++ 中，`delete this` 是合法操作，但它就像是在“走钢丝”。它的核心逻辑是：**当前对象请求销毁自己**。

如果不极其小心，它会直接导致程序崩溃或未定义行为（Undefined Behavior）。以下是 `delete this` 会出问题的几种典型情况：

1. **对象不在堆（Heap）上**

这是最致命的问题。`delete` 只能用于通过 `new` 分配的对象。

- **错误场景**：如果对象是在栈上创建的局部变量，或者是全局变量，调用 `delete this` 会试图释放非堆内存。
- **后果**：程序会立即崩溃（通常是 Segmentation Fault 或 Heap Corruption）。

2. **成员访问“死灰复燃”（Use After Free）**

一旦执行了 `delete this`，当前对象的所有成员变量和虚函数表指针都会失效。

- **错误场景**：在 `delete this` 之后，继续访问类的成员变量或调用非静态成员函数。
- **后果**：虽然函数代码可能还在内存里，但数据可能已经被系统回收或篡改，导致逻辑错误或崩溃。

3. **多重释放（Double Free）**

- **错误场景**：如果一个对象既执行了 `delete this`，随后又被外部的智能指针（如 `std::unique_ptr`）或者手动 `delete` 释放。
- **后果**：典型的双重释放错误，会导致堆损坏。

4. **析构函数中的递归循环**

- **错误场景**：如果在析构函数中直接或间接地触发了 `delete this`。
- **后果**：无限递归导致栈溢出（Stack Overflow）。

**`delete this` 的生存指南**

虽然危险，但在某些特定场景（如引用计数智能指针的内部实现、异步回调销毁）中确实会用到。如果你必须使用它，请确保满足以下条件：

1. **绝对保证**对象是通过 `new` 创建的。
2. **绝对保证**在 `delete this` 之后，当前函数体内不再访问任何成员，且立即 `return`。
3. **绝对保证**外部没有任何其他指针再指向这个对象。
