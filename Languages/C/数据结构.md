...
int main(int argc, char* argv[])
{
    ...
    {
        node* cursor = root;  // 光标：遍历trie

        for (int i = 0, n = strlen(name); i < n; i++)
        {
            int index = tolower(name[i]) - 'a'; // 索引：暂存处理的字母
            // 延伸trie
            if (cursor->children[index] == NULL)
            {
                ...
                cursor->children[index] = new;  //  (1)让光标指向的子指针指向下一个节点
            }

            // Go to node which we may have just been made
            cursor = cursor->children[index]; // (2)让光标指向下一个节点
        }

        // if we are at the end of the word, mark it as being a word
        cursor->is_word = true;  // 此时cursor已经更新到下一个子指针
    }
    ...
    for (int i = 0; word[i] != '\0'; i++)
    {
        ...
        if (cursor->children[index] != NULL)
        {
            ...
            if (cursor->is_word == true)
                return true;  // 找到完整的单词，直接返回true    （true在单词最后一个字母子指针的后面！！！）
        }
    }
    return false;
}
